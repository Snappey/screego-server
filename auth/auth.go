...
func (u *Users) CurrentUser(r *http.Request, conf config.Config) (string, bool) {
    if conf.AuthMode == config.AuthModeProxy {
        usernameHeader := conf.UsernameProxyHeader

        if proxyUsername := r.Header.Get(usernameHeader); proxyUsername != "" {
            return proxyUsername, true
        }
    }

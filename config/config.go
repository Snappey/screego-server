...
type Config struct {
    ...
    UsernameProxyHeader string   `split_words:"true"`
...
func Get() (Config, []FutureLog) {
    ...
    if config.AuthMode != AuthModeTurn && config.AuthMode != AuthModeAll && config.AuthMode != AuthModeNone && config.AuthMode != AuthModeProxy {
        logs = append(logs,
